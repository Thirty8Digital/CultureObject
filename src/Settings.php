...
class Settings extends Core {
				...
				if ( ! method_exists( $provider_class, 'register_remappable_fields' ) ) {
					...
					if ( current_theme_supports( 'cos-remaps' ) && $fields ) {
						foreach ( $fields as $field_key => $field_default ) {
							...
							add_settings_field(
								'cos_remap_' . strtolower( $field_key ),
								$field_key,
								array( $this, 'generate_settings_field_input_text' ),
								'cos_remap_settings',
								'cos_remaps',
								array(
									'field'   => 'cos_remap_' . strtolower( $field_key ),
	...
	function provide_load_action() {
		...
		if ( $provider ) {
			...
			$provider_class = new $provider['class']();
			if ( method_exists( $provider_class, 'execute_load_action' ) ) {
				$provider_class->execute_load_action();
			}
		}
	}
	...
	function provide_init_action() {
		...
		if ( $provider ) {
			...
			$provider_class = new $provider['class']();
			if ( method_exists( $provider_class, 'execute_init_action' ) ) {
				$provider_class->execute_init_action();
			}
		}
	}
