...
class SWCE extends \CultureObject\Provider {
	...
	function register_settings() {
		...
		add_settings_field(
			'cos_provider_category_slug',
			__( 'SWCE Category Slug', 'culture-object' ),
			array( $this, 'generate_settings_field_input_text' ),
			'cos_provider_settings',
			'cos_provider_settings',
			array(
				'field'       => 'cos_provider_category_slug',
	...
	function create_object( $doc ) {
		...
		if ( ! $term ) {
			$term = wp_insert_term( $doc['category']['name'], 'object_category' );
		}
		if ( $term ) {
			$tax['object_category'] = intval( $term['term_id'] );
		}
		unset( $doc['category'] );
