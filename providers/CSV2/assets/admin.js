...
$(document).ready(function() {
    ...
    $('#csv_perform_ajax_import').click(function(e) {
        ...
        importChunk(1, $('#csv_perform_ajax_import').data('import-id'), $('#csv_perform_ajax_import').data('sync-key'), $('#csv_perform_ajax_import').data('starting-nonce'), id_field, title_field, perform_cleanup);
    });
})


function importChunk(start, import_id, sync_key, nonce, id_field, title_field, perform_cleanup) {
    $.post(
        ajaxurl, 
        {
            'action': 'cos_sync',
        ...
        function(response) {
            if (response.state == "error") {
                ...
                if (response.complete) {
                    ...
                    $("#csv_import_detail").prepend(response.import_status.join("<br />")+"<br />");
                    importChunk(response.next_start, import_id, sync_key, response.next_nonce, id_field, title_field, perform_cleanup);
                }
            }
        },
        'json'
