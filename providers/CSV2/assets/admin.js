...
$(document).ready(function() {
    ...
    $('#csv_perform_ajax_import').click(function(e) {
        ...
        importChunk(1, $('#csv_perform_ajax_import').data('sync-key'), $('#csv_perform_ajax_import').data('starting-nonce'));
    });
})


function importChunk(start, sync_key, nonce) {
    $.post(
        ajaxurl, 
        {
            'action': 'cos_sync',
        ...
        function(response) {
            if (response.complete) {
                ...
                $("#csv_import_detail").prepend(response.import_status.join("<br />")+"<br />");
                importChunk(response.next_start, sync_key, response.next_nonce);
            }
        },
        'json'
    ); 
